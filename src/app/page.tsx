...
import { visibleProducts, categories } from "@/data/catalog";

const Home = () => {
  const { t } = useI18n();
  const featured = visibleProducts.filter((p) => p.isFeatured).slice(0, 4);
  const newArrivals = visibleProducts.filter((p) => p.isNew).slice(0, 3);
  ...
  return (
    <>
      ...
      <section className="container-wide py-20 lg:py-28">
        ...
        <div className="mt-10 flex snap-x snap-mandatory gap-5 overflow-x-auto pb-4 sm:grid sm:grid-cols-2 sm:overflow-visible sm:pb-0 lg:grid-cols-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {categories.map((cat) => (
            <Link
              key={cat.id}
              href={`/products?category=${cat.id}`}
              className="group relative aspect-[4/3] w-[85vw] shrink-0 snap-center overflow-hidden rounded-xl border border-border/60 sm:w-auto"
            >
              <img
                src={cat.image}
                alt={cat.name}
